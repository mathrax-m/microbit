//Ch09-2-2
//  micro:bit+Processing+SonicPi
//  -- 加速度センサX,1つだけをつかってSonicPiと連携 --
...
void drawMain() {
  ...
  //microbitDataを、整数型の0〜12に整える  
  int x = (int) map(microbitData[0], -1024, 1023, 0, 12);
  
  //センサの値で鍵盤を操作する
  for (int i=0; i<13; i++) {
    if (i == x) {            //鍵盤の番号と一致したら
      bright[i]=1.0;         //鍵盤を押す色変化の変数
      sendOscSonicPi(i+72);  //鍵盤の番号+72を、SonicPiへOSC送信（oscタブ内）
    }
  }
